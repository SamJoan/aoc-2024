...
def corrupt!(corruption, memory_map, kilobytes)
  kb_read = 0
  corruption.each do |x, y|
...
def navigate_map(memory_map, start_position)
  ...
  next_pos_right = step(memory_map, start_position, :right)
  next_pos_right_x, next_pos_right_y = next_pos_right
  mh << Element.new(1, next_pos_right) if memory_map[next_pos_right_y][next_pos_right_x] != '#'
...
def read_corruption(filename)
  corruption = []
  IO.readlines(filename).map(&:strip).each do |line|
    coords = line.split(',').map(&:to_i)
    corruption.append(coords)
  end

  corruption
end

corruption = read_corruption(ARGV[1])
(0..corruption.length).each do |nb|
  puts "[+] Navigating #{nb}" if nb  % 10 == 0 && nb != 0
  memory_map = generate_memory_map(ARGV[0])
  corrupt!(corruption, memory_map, nb)

  start_pos = [0, 0]
  solvable = navigate_map(memory_map, start_pos)
  if !solvable
    puts "Unsolvable after index #{nb}, line contents #{corruption[nb - 1]}"
    break
  end
end
