...
def reorder_invalid_update(update, rules)
  rules.each do |rule|
    before, after = rule
    before_pos = update.index(before)
    after_pos = update.index(after)
    ...
    if both_elements_present && !(before_pos < after_pos)
      ...
      new_update[after_pos] = before
      new_update[before_pos] = after
      
      return reorder_invalid_update(new_update, rules)
    end
  end
...
updates.each do |update|
  if !is_update_valid?(update, rules)
    ...
    reordered = reorder_invalid_update(update, rules)
    total += reordered[middle_index].to_i
  end
end
